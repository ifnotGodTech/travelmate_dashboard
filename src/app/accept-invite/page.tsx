...
const LoginComponent = () => {
  ...
  const handleSubmit = async (values: { password1: string; password2: string }) => {
    setLoading(true);
    // Simulate API request or call real API here
    console.log("Valid password submitted:", values.password1);
      ...
      >
        {({ values, isValid }) => {
          const validations = {
            ...
            match: values.password1 === values.password2 && values.password2 !== "",
          };

          return (
            <div className="bg-[#fff] p-[40px] space-y-10 rounded-[20px]">
              <div className="flex flex-col items-center gap-4">
                <img src="/assets/images/logo.svg" alt="" className="lg:w-28 w-[53px]" />
                <p className="text-[#181818] lg:text-2xl text-[18px] font-semibold">
                  Complete your Account Setup
                </p>
                <p className="text-[#181818] lg:text-lg text-[16px] font-medium">
                  You've been invited to join the TravelMate administrative team. Please
              ...
              <Form className="space-y-10">
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-2">
                    <ValidationItem isValid={validations.uppercase} text="At least one uppercase letter (A-Z)" />
                    <ValidationItem isValid={validations.lowercase} text="At least one lowercase letter (a-z)" />
                    <ValidationItem isValid={validations.number} text="At least one number (0-9)" />
                    <ValidationItem isValid={validations.specialChar} text="At least one special character (!@#$%^&*()_-+=<>?/{}[]|)" />
                    <ValidationItem isValid={validations.match} text="Passwords must match" />
                  </div>
                </div>
              </Form>
            </div>
...
const Inputs = () => {
  return (
    <div className="space-y-10">
      <InputReusable label="New Password" placeholder="Enter your password" name="password1" />
      <InputReusable label="Confirm Password" placeholder="Re-enter password" name="password2" />
    </div>
  );
};
...
const InputReusable = ({
  ...
  return (
    <div className="space-y-4 relative">
      <p className="text-[#181818] lg:text-[16px] font-semibold text-[14px]">{label}</p>
